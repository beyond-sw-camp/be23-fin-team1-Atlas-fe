...
const markerElements: Array<{ node: MonitoringMapNode; element: HTMLElement }> = []
const loadError = ref('')
...
onMounted(async () => {
  ...
  try {
    ...
    props.nodes.forEach((node, index) => {
      ...
      markerElements.push({ node, element: marker })
      markers.push(mapMarker)
    })
  } catch (error) {
    console.error('Failed to initialize MapLibre', error)
    loadError.value = '인터랙티브 지도 로드에 실패했습니다.'
  }
})
...
<template>
  <div class="supplier-map supplier-map--raster">
