...
<template>
  <main class="login-screen">
    <section class="login-card">
      <div class="login-card__eyebrow">{{ UI_COPY.secureAccess[preferences.language] }}</div>

      <!-- 일반 로그인 단계입니다. -->
      <template v-if="!session.loginVerificationRequired">
        <h1>{{ UI_COPY.loginTitle[preferences.language] }}</h1>
        <p>{{ UI_COPY.loginDescription[preferences.language] }}</p>

        <form class="login-form" @submit.prevent="session.signIn">
          <label>
            <span>{{ UI_COPY.loginId[preferences.language] }}</span>
            <input v-model="session.loginId" type="text" autocomplete="username" />
          </label>

          <label>
            <span>{{ UI_COPY.loginPassword[preferences.language] }}</span>
            <input
          ...
          <button type="submit">{{ UI_COPY.loginButton[preferences.language] }}</button>
        </form>

        <div class="login-hint">{{ UI_COPY.loginHint[preferences.language] }}</div>
        <div v-if="session.loginError" class="login-error">{{ session.loginError }}</div>
      </template>

      <!-- 새 IP 이메일 인증 단계입니다. -->
      <template v-else>
        <h1>{{ preferences.language === 'ko' ? '이메일 인증' : 'Email Verification' }}</h1>

        <p>
          {{
            preferences.language === 'ko'
              ? '새로운 IP에서 로그인이 감지되었습니다. 이메일로 받은 인증 코드를 3분 안에 입력해 주세요.'
              : 'A sign-in from a new IP was detected. Please enter the verification code sent to your email within 3 minutes.'
          }}
        </p>

        <form class="login-form" @submit.prevent="session.verifyLoginIp">
          <label>
            <span>{{ preferences.language === 'ko' ? '인증 코드' : 'Verification Code' }}</span>
            <input
          ...
          <button type="submit">
            {{ preferences.language === 'ko' ? '인증하기' : 'Verify' }}
          </button>
        </form>
        ...
        <div class="login-hint">
          {{
            preferences.language === 'ko'
              ...
              : `Verification expires at: ${formattedVerificationExpiresAt}`
          }}
        </div>
        ...
        >
          {{ preferences.language === 'ko' ? '다시 로그인하기' : 'Back to Sign In' }}
        </button>
      </template>
    </section>
  </main>
