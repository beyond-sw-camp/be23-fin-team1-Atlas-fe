...
const currentUserDisplayName = computed(() => {
  ...
  return me ? getInviteUserDisplayName(me) : sessionStore.loginId || copy.value.me
})
...
const filteredAvailableUsers = computed(() => {
  ...
  if (q) {
    users = users.filter((u) => getInviteUserDisplayName(u).toLowerCase().includes(q))
  }
  return users
})
...
async function handleCreateRoom() {
  ...
  const selectedDisplayNames = selectedUsers.value.map(id => {
    ...
    return user ? getInviteUserDisplayName(user) : 'Unknown'
  })
      ...
      <div class="chat-room-list__create-users">
        <label v-for="user in filteredAvailableUsers" :key="user.userPublicId" class="chat-room-list__invite-user">
          ...
          <ChatAvatar
            ...
            :name="getInviteUserDisplayName(user)"
            size="sm"
          />
          <span class="chat-room-list__invite-copy">
            <span>{{ getInviteUserDisplayName(user) }}</span>
            <span v-if="getInviteUserMeta(user)" class="chat-room-list__invite-role">{{ getInviteUserMeta(user) }}</span>
          </span>
        </label>
